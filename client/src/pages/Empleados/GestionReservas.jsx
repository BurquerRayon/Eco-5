...
const GestionReservas = () => {
  ...
  const cambiarEstadoReserva = async (id, nuevoEstado) => {
    const mensajeConfirmacion = nuevoEstado === 'cancelado' 
      ...
      : `¿Seguro que deseas marcar esta reserva como ${nuevoEstado}?`;
  ...
  const exportarPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text('Reporte de Reservas - Empleado', 14, 15);
    ...
    doc.save('reporte-reservas-empleado.pdf');
  };
  ...
  const exportarExcel = () => {
    ...
    saveAs(data, 'reservas-empleado.xlsx');
  };

  return (
    <div className="admin-reservas-container">
      <Link to="/Home/HomeEmployee" className="btn-volver">Volver al Panel de Empleado</Link>
      <h2>Gestión de Reservas - Empleado</h2>
            ...
            <tr key={i}>
              ...
              <td>
                {r.estado === 'pendiente' && (
                  <>
                    <button 
                      onClick={() => cambiarEstadoReserva(r.id_reserva, 'confirmado')}
                      title="Confirmar reserva"
                    >
                      ✅
                    </button>
                    <button 
                      onClick={() => cambiarEstadoReserva(r.id_reserva, 'cancelado')}
