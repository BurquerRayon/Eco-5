...
const RolesConfig = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editandoId) {
        await axios.put(`http://ecomaravilla2.duckdns.org:3001/api/roles/${editandoId}`, form);
        setMensaje('✅ Rol actualizado');
      } else {
        await axios.post('http://ecomaravilla2.duckdns.org:3001/api/roles', form);
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            try {
              await axios.delete(`http://ecomaravilla2.duckdns.org:3001/api/roles/${id}`);
